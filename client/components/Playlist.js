...
export default function Playlist({ navigation }) {
  ...
  const addSongToFavorites = async (song) => {
    const res = await fetch(
      `http://192.168.0.128:3000/api/v1/playlists/${defaultPlaylists["My Favorites"]._id}/addSong`,
      {
        method: "PATCH",
        headers: {
  ...
  const deleteSongFromFavorites = async (song) => {
    const res = await fetch(
      `http://192.168.0.128:3000/api/v1/playlists/${defaultPlaylists["My Favorites"]._id}/deleteSong`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
